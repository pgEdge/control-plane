...
COMMAND:
    ...
    cancel-database-task: Cancels a running or pending task for a database.
...
Example:
    %[1]s control-plane get-host --host-id "host-1"
`, os.Args[0])
}
...
Example:
    %[1]s control-plane get-database --database-id "my-app"
`, os.Args[0])
}
...
Example:
   ...
   }' --database-id "my-app" --force-update true
`, os.Args[0])
}
...
func controlPlaneSwitchoverDatabaseNodeUsage() {
	fmt.Fprintf(os.Stderr, `%[1]s [flags] control-plane switchover-database-node -body JSON -database-id STRING -node-name STRING

Performs a planned switchover for a node's primary to a replica candidate.
    -body JSON: 
    -database-id STRING: ID of the database to operate on.
    -node-name STRING: Name of the node to operate on.
...
Example:
    %[1]s control-plane list-database-tasks --database-id "my-app" --after-task-id "3c875a27-f6a6-4c1c-ba5f-6972fb1fc348" --limit 100 --sort-order "ascend"
`, os.Args[0])
}
...
Example:
    %[1]s control-plane get-database-task --database-id "my-app" --task-id "3c875a27-f6a6-4c1c-ba5f-6972fb1fc348"
`, os.Args[0])
}
...
Returns the log of a particular task for a database.
    -database-id STRING: ID of the database to get the task log for.
    -task-id STRING: ID of the task to get the log for.
    -after-entry-id STRING: 
    -limit INT: 

Example:
    %[1]s control-plane get-database-task-log --database-id "my-app" --task-id "3c875a27-f6a6-4c1c-ba5f-6972fb1fc348" --after-entry-id "3c875a27-f6a6-4c1c-ba5f-6972fb1fc348" --limit 100
`, os.Args[0])
}
...
func controlPlaneCancelDatabaseTaskUsage() {
	fmt.Fprintf(os.Stderr, `%[1]s [flags] control-plane cancel-database-task -database-id STRING -task-id STRING

Cancels a running or pending task for a database.
    -database-id STRING: ID of the database that owns the task.
    -task-id STRING: ID of the task to cancel.

Example:
    %[1]s control-plane cancel-database-task --database-id "abc123" --task-id "def456"
`, os.Args[0])
}
