...
func UsageCommands() []string {
	return []string{
		"control-plane (init-cluster|join-cluster|get-join-token|get-join-options|get-cluster|list-hosts|get-host|remove-host|list-databases|create-database|get-database|update-database|apply-upgrade|apply-major-upgrade|delete-database|backup-database-node|switchover-database-node|failover-database-node|list-database-tasks|get-database-task|get-database-task-log|list-host-tasks|get-host-task|get-host-task-log|list-tasks|restore-database|get-version|restart-instance|stop-instance|start-instance|cancel-database-task)",
	}
}
...
func ParseEndpoint(
	...
	var (
		...
		controlPlaneApplyMajorUpgradeFlags          = flag.NewFlagSet("apply-major-upgrade", flag.ExitOnError)
		controlPlaneApplyMajorUpgradeBodyFlag       = controlPlaneApplyMajorUpgradeFlags.String("body", "REQUIRED", "")
		controlPlaneApplyMajorUpgradeDatabaseIDFlag = controlPlaneApplyMajorUpgradeFlags.String("database-id", "REQUIRED", "ID of the database to upgrade.")
	...
	controlPlaneApplyMajorUpgradeFlags.Usage = controlPlaneApplyMajorUpgradeUsage
	...
	)
	{
		...
		case "control-plane":
			...
			case "apply-major-upgrade":
				epf = controlPlaneApplyMajorUpgradeFlags
	...
	)
	{
		...
		case "control-plane":
			...
			case "apply-major-upgrade":
				endpoint = c.ApplyMajorUpgrade()
				data, err = controlplanec.BuildApplyMajorUpgradePayload(*controlPlaneApplyMajorUpgradeBodyFlag, *controlPlaneApplyMajorUpgradeDatabaseIDFlag)
			case "delete-database":
...
func controlPlaneUpdateDatabaseUsage() {
	...
	fmt.Fprint(os.Stderr, " -remove-host JSON")
	fmt.Fprintln(os.Stderr)
	...
	fmt.Fprintln(os.Stderr, `    -remove-host JSON: `)
...
func controlPlaneApplyMajorUpgradeUsage() {
	// Header with flags
	fmt.Fprintf(os.Stderr, "%s [flags] control-plane apply-major-upgrade", os.Args[0])
	fmt.Fprint(os.Stderr, " -body JSON")
	fmt.Fprint(os.Stderr, " -database-id STRING")
	fmt.Fprintln(os.Stderr)
	...
	// Description
	fmt.Fprintln(os.Stderr)
	fmt.Fprintln(os.Stderr, `Applies a dedicated Spock major-version upgrade (e.g. 5.x to 6.x) to a database, rolling through each node one at a time rather than redeploying uniformly. Independent of apply-upgrade, which is restricted to same-major minor/patch bumps and cannot reach a different Spock major.`)
