...
func TestToVersion(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			in:       "2:9.1.0016-1ubuntu7.12",
			expected: &ds.Version{Components: []uint64{9, 1, 16}},
		},
		{
			in:       "3.12.3-1ubuntu0.13",
			expected: &ds.Version{Components: []uint64{3, 12, 3}},
		},
		{
			in:       "17.9-1.noble",
			expected: &ds.Version{Components: []uint64{17, 9}},
		},
	} {
...
func MockExecCommand(t testing.TB, mockStdout, mockStderr string, mockErr error) ExecCommand {
	t.Helper()

	return func(_ context.Context, stdout, stderr io.Writer, name string, args ...string) error {
		if mockStdout != "" {
			_, err := stdout.Write([]byte(mockStdout))
			require.NoError(t, err)
		}
		if mockStderr != "" {
			_, err := stderr.Write([]byte(mockStderr))
			require.NoError(t, err)
		}
		return mockErr
	}
