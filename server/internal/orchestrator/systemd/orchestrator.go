...
func (o *Orchestrator) PopulateHost(ctx context.Context, h *host.Host) error {
	...
	for _, v := range versions {
		o.logger.Debug().
			Str("postgres_name", v.Postgres.Name).
			Str("postgres_major", v.Postgres.PostgresMajor).
			Str("postgres_version", v.Postgres.Version.String()).
			Msg("postgres version")

		for _, s := range v.Spock {
			o.logger.Debug().
				Str("spock_name", s.Name).
				Str("spock_postgres_major", s.PostgresMajor).
				Str("spock_version", s.Version.String()).
				Msg("spock version")
		}
	}

	var supported []*host.PgEdgeVersion
	...
	slices.SortFunc(supported, func(a, b *host.PgEdgeVersion) int {
...
func validatePort(previous, current *int) error {
	if current == nil || *current == 0 {
		return errors.New("port must be defined and non-zero")
	}
