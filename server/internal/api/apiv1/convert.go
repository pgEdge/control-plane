...
func databaseNodesToAPI(nodes []*database.Node) []*api.DatabaseNodeSpec {
	...
	for i, node := range nodes {
		...
		apiNodes[i] = &api.DatabaseNodeSpec{
			...
			SpockVersion:     utils.NillablePointerTo(node.SpockVersion),
			Port:             node.Port,
			...
			PgHbaConf:        node.PgHbaConf,
			PgIdentConf:      node.PgIdentConf,
...
func databaseSpecToAPI(d *database.Spec) *api.DatabaseSpec {
	return &api.DatabaseSpec{
		...
		PgHbaConf:        d.PgHbaConf,
		PgIdentConf:      d.PgIdentConf,
...
func apiToDatabaseNodes(apiNodes []*api.DatabaseNodeSpec) ([]*database.Node, error) {
	...
	for i, apiNode := range apiNodes {
		...
		nodes[i] = &database.Node{
			...
			SpockVersion:     utils.FromPointer(apiNode.SpockVersion),
			Port:             apiNode.Port,
			PatroniPort:      apiNode.PatroniPort,
			CPUs:             cpus,
			MemoryBytes:      memory,
			PostgreSQLConf:   apiNode.PostgresqlConf,
			PgHbaConf:        apiNode.PgHbaConf,
			PgIdentConf:      apiNode.PgIdentConf,
...
func apiToDatabaseSpec(
	...
	return &database.Spec{
		...
		Port:             apiSpec.Port,
		PatroniPort:      apiSpec.PatroniPort,
		CPUs:             cpus,
		MemoryBytes:      memory,
		Nodes:            nodes,
		DatabaseUsers:    users,
		...
		PgHbaConf:        apiSpec.PgHbaConf,
		PgIdentConf:      apiSpec.PgIdentConf,
		RestoreConfig:    restoreConfig,
