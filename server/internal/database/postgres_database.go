...
func (p *PostgresDatabaseResource) Create(ctx context.Context, rc *resource.Context) error {
	...
	if p.isFirstTimeSetup(rc) && p.HasRestoreConfig {
...
func (p *PostgresDatabaseResource) isFirstTimeSetup(rc *resource.Context) bool {
	// This database will already exist in the state if it's been successfully
	// created before. It's important that we use State.Get here rather than
	// FromContext/FromState so that we can ignore the NeedsRecreate flag on the
	// resource data.
	_, ok := rc.State.Get(p.Identifier())
	return !ok
}
