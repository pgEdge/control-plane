...
func (h *Host) Stop(ctx context.Context) error {
	// Allow more time for graceful container shutdown (default is 10s)
	stopTimeout := 30 * time.Second
	return h.container.Stop(ctx, &stopTimeout)
}
...
func createHost(ctx context.Context, config HostConfig, network *testcontainers.DockerNetwork, image string, initialPeers []string) (*Host, error) {
	...
	// This allows tests to start fresh while reusing certificates for performance
	...
	// Set certificate directory path
	// Certificates will be auto-generated by the control-plane on first startup if they don't exist
